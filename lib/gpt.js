...
GPT.prototype = {
  ...
  /**
   * Move the backup GPT to another LBA. In the process, also resize
   * the last partition so that it ends just before the backup GPT.
   * @param {Number} [newLBA] - this is the LBA of the backup GPT header
   */
  moveBackup( newLBA ) {
    // Check whether this is a primary or a backup GPT
    var isPrimary = this.backupLBA == 0 || this.currentLBA < this.backupLBA

    // Set new LBA of the backup GPT
    if( isPrimary ) {
      this.backupLBA = newLBA
    } else {
      this.currentLBA = newLBA
      this.tableOffset = newLBA - 32
    }

    // Set last LBA
    this.lastLBA = newLBA - 33

    // Set last LBA of the last partition
    var nparts = this.partitions.length
    if( nparts > 0 ) {
      this.partitions[nparts - 1].lastLBA = this.lastLBA
    }
  }
